    ...
    SignInManager<AppUser> signInManager) : ControllerBase
{
    ...
    public async Task<IActionResult> LoginWithSignInManager(LoginDto request, CancellationToken cancellationToken)
    {
        ...
        if (result.IsLockedOut)
        {
            ...
            {
                return StatusCode(500, $"Şifrenizi 3 kere yanlış girdiğiniz için kullanıcınız {timeSpan.Value.TotalSeconds} saniye girişi engellenmiştir. Süre bitiminde tekrar giriş yapabilirsiniz.");
            }
            else
            {
                return StatusCode(500, $"Şifrenizi 3 kere yanlış girdiğiniz için kullanıcınız 30 saniye girişi engellenmiştir. Süre bitiminde tekrar giriş yapabilirsiniz.");
            }
        }
